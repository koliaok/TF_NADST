...
class NADST():
    ...
    def model(self, xs, ys, src_lang, domain_lang, slot_lang, len_val, args, training=True):
        ...
        with tf.compat.v1.variable_scope('nadst', reuse=tf.compat.v1.AUTO_REUSE):
            ...
            fertility_generator, point_state_generator, gate_gen = self.get_generators(len_val, src_vocab, text_embedding, args)
            ...
            self.calculation_NADST(encoder=encoder, context_mask=context_masks_, delex_context_masks=delex_context_masks_,
                                   ...
                                   fertility_generator=fertility_generator, state_generator=point_state_generator,
            ...
            evaluation_variable = {'sorted_lenval': sorted_lenvals,
                                   'sorted_generate_y': sorted_generate_ys,
                                   'sorted_in_domains2': sorted_in_domains2s,
                                   'sorted_in_slots2': sorted_in_slots2s,
                                   'lenval_out': lenval_out_from_gen,
                                   'state_out': state_out_from_gen,
                                   'gate_out': gate_out_from_gen,
                                   'turn_id': turn_ids,
                                   'context_plain': context_plains,
                                   'ID': ids,
                                   'turn_id': turn_ids,
                                   'turn_belief': turn_beliefs}
    ...
    def calculation_NADST(self, encoder, context_mask, delex_context_masks, sorted_in_domainslots_masks,
        ...
        fertility_out = self.fertility_decoder.get_fertility_decoder(context_mask, delex_context_masks, args, src_masks, training=training)

        self.state_decoder = State_Decoder(encoder, state_generator)
        self.state_out = self.state_decoder.get_state_decoder(fertility_out, context_mask,
    ...
    def compute_loss(self, sorted_lenval, sorted_gates, contexts, context_masks, sorted_generate_y, d_model, factor, warmup, gate_gen):
        with tf.compat.v1.variable_scope('comput_loss', reuse=tf.compat.v1.AUTO_REUSE):
            ...
            state_out, state_out_size = self.state_decoder.state_generator.get_point_generator(out_states=self.state_out['out_states'],
            ...
            state_logits = tf.compat.v1.reshape(state_out, shape=[-1, state_out_size])
        ...
        return total_loss, train_op, global_step, summaries, losses, nb_tokens, self.state_out, lenval_out, state_out, gate_out
